...
export default class ics {
  constructor(){
    this.calendarStart = ['BEGIN:VCALENDAR', 'PRODID:Calendar', 'VERSION:2.0'].join(SEPARATOR),
    this.calendarEvents = [],
   ...
   */
  addEvent(subject, description, location, url, begin, stop, timezone, rrule) {
    ...
    if (timezone) {
      for (let k = 0; k < tz.length; k++) {
        ...
        if (tz[k].name === timezone) {
          
          timezoneContent = tz[k].ical;
        }
      }
    }
    ...
    var calendarEvent = [
      'BEGIN:VEVENT',
      'UID:' + this.calendarEvents.length + "@default",
      'CLASS:PUBLIC',
    ...
    if (timezone) {
      console.log(timezoneContent)
      calendarEvent.splice(0, 0, timezoneContent);
    }
  ...
  download(filename, ext) {
    ...
    var calendar = this.calendarStart + this.SEPARATOR + this.calendarEvents.join(SEPARATOR) + this.calendarEnd;
